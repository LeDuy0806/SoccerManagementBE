...
export class MatchService {
  public async getMatches(): Promise<IMatch[]> {
    try {
      ...
      if (!matches) {
        throw new HttpException(HTTP_STATUS.NOT_FOUND, `matches not found`);
      }
      return matches;
    } catch {
  ...
  public async getMatch(id: string): Promise<IMatch> {
    try {
      ...
      if (!match) {
        throw new HttpException(HTTP_STATUS.NOT_FOUND, `match not found`);
      }
      return match;
    } catch {
      throw new HttpException(HTTP_STATUS.INTERNAL_SERVER_ERROR, `Server error`);
  ...
  public async createMatch(MatchData: IMatch): Promise<IMatch> {
    ...
    if (existsMatchName) {
      throw new HttpException(HTTP_STATUS.UNPROCESSABLE_ENTITY, `Match already exists`);
    }
  ...
  public async updateMatch(matchData: IMatch, id: string): Promise<IMatch> {
    ...
    if (!ObjectId.isValid(id)) {
      throw new HttpException(HTTP_STATUS.NOT_FOUND, `No Match with id: ${id}`);
    }
