...
export class SponsorRepository {
  public async getSponsors(): Promise<ISponsor[]> {
    try {
      ...
      if (!sponsors) {
        throw new HttpException(HTTP_STATUS.NOT_FOUND, `sponsors not found`);
      }
      return sponsors;
    } catch {
  ...
  public async getSponsor(id: string): Promise<ISponsor> {
    try {
      ...
      if (!sponsor) {
        throw new HttpException(HTTP_STATUS.NOT_FOUND, `sponsor not found`);
      }
      return sponsor;
    } catch {
      throw new HttpException(HTTP_STATUS.INTERNAL_SERVER_ERROR, `Server error`);
  ...
  public async createSponsor(sponsorData: ISponsor): Promise<ISponsor> {
    ...
    if (existsSponsorName) {
      throw new HttpException(HTTP_STATUS.UNPROCESSABLE_ENTITY, `Sponsor already exists`);
    }
  ...
  public async updateSponsor(sponsorData: ISponsor, id: string): Promise<ISponsor> {
    const { name, logo, email } = sponsorData;

    if (!ObjectId.isValid(id)) {
      throw new HttpException(HTTP_STATUS.NOT_FOUND, `No Sponsor with id: ${id}`);
    }
    ...
    try {
      const updateSponsor = await Sponsor.findByIdAndUpdate(id, newSponsor, {
        new: true,
      });
