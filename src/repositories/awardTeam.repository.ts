...
export class AwardTeamRepository {
  public async getAwardTeams(): Promise<IAwardTeam[]> {
    try {
      ...
      if (!awardTeams) {
        throw new HttpException(HTTP_STATUS.NOT_FOUND, `awardTeams not found`);
      }
      return awardTeams;
    } catch {
  ...
  public async getTeam(id: string): Promise<IAwardTeam> {
    try {
      ...
      if (!awardTeam) {
        throw new HttpException(HTTP_STATUS.NOT_FOUND, `awardTeam not found`);
      }
      return awardTeam;
    } catch {
      throw new HttpException(HTTP_STATUS.INTERNAL_SERVER_ERROR, `Server error`);
  ...
  public async updateTeam(teamData: IAwardTeam, id: string): Promise<IAwardTeam> {
    const { name } = teamData;
    ...
    try {
      const updateTeam = await AwardTeam.findByIdAndUpdate(id, newAwardTeam, {
        new: true,
      });
      return updateTeam!;
    } catch {
